  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white">
        {onBack && (
          <button onClick={onBack} className="p-1 rounded-lg hover:bg-slate-100 transition sm:hidden">
        ...
        <div>
          <div className="font-semibold text-slate-900 text-sm">{otherUser?.full_name || 'Utente'}</div>
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="flex items-center gap-2 px-4 py-3 border-t border-slate-100 bg-white">
        <input
          ref={inputRef}
          type="text"
          value={newMessage}
          ...
          className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          maxLength={2000}
        />
        <button
          type="submit"
    ...
    <div className="flex flex-wrap gap-2 px-4 py-2">
      {templates.map((t) => (
        <button
          key={t}
          onClick={() => onSelect(t)}
          className="text-xs px-3 py-1.5 bg-slate-100 text-slate-600 rounded-full hover:bg-slate-200 transition"
        >
          {t}
        </button>
      ))}
