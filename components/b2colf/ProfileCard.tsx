...
export default function ProfileCard({ id, full_name, city, bio, rating_avg, verified, role }: Props) {
  ...
  return (
    <article className="border border-slate-100 rounded-xl p-4 hover:shadow-soft transition group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              <Link href={`/profile/${id}`} className="font-semibold text-slate-900 group-hover:text-primary transition truncate block">
                {full_name}
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => toggleFavorite(id)} className="p-1 hover:bg-slate-50 rounded transition mr-1">
                <Heart className={`h-4 w-4 ${isFavorite(id) ? 'fill-danger text-danger' : 'text-slate-300'}`} />
              </button>
              <Star className="h-4 w-4 text-accent fill-accent" />
              <span className="text-sm font-semibold text-slate-700">{rating_avg?.toFixed(1)}</span>
            </div>
          </div>
      ...
      <div className="mt-4 flex justify-end">
        <Link href={`/profile/${id}`}>
          <Button className="text-sm rounded-lg">Vedi profilo</Button>
        </Link>
      </div>
    </article>
  )
