...
export default function GigsList() {
  ...
  return (
    <>
      <div className="text-sm text-slate-500 mb-3">{sorted.length} annunci trovati</div>
      ...
      {!sorted.length ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-slate-500 mt-2 max-w-sm mx-auto">Prova a modificare i filtri o la ricerca, oppure pubblica un nuovo annuncio.</p>
          <Link href="/gigs/new" className="inline-flex items-center gap-2 mt-4 px-5 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-600 transition">
            <PlusCircle className="h-4 w-4" /> Pubblica annuncio
          </Link>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {paginated.map((g) => (
              <article key={g.id} className="border border-slate-100 rounded-xl p-4 hover:shadow-soft transition group">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <Link href={`/gigs/${g.id}`} className="font-semibold text-slate-900 group-hover:text-primary transition">
                      {g.title}
                    </Link>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggleFavorite(g.id)} className="p-1 hover:bg-slate-50 rounded transition">
          ...
          {page < totalPages && (
            <div className="mt-6 text-center">
              <button onClick={() => setPage(p => p + 1)} className="px-6 py-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition">
      ...
      {applying && (
        <Dialog open={true} onOpenChange={() => setApplying(null)} title={`Candidati per: ${applying.title}`}>
          <p className="text-sm text-slate-500">Prezzo indicativo: &euro;{applying.price}</p>
          ...
          <div className="mt-4 flex gap-2 justify-end">
            <button className="px-4 py-1.5 border border-slate-200 rounded-lg text-sm" onClick={() => setApplying(null)}>Annulla</button>
