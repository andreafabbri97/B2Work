...
export default function GigsIndexPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Filters bar */}
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl p-4 mb-6 flex flex-col sm:flex-row gap-3">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {sorted.map((g) => (
            <Link key={g.id} href={`/gigs/${g.id}`} className="block border border-slate-100 dark:border-slate-700 rounded-xl p-5 hover:shadow-soft transition group">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-primary transition">{g.title}</h3>
